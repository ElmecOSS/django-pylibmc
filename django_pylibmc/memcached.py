...
if MIN_COMPRESS_LEN > 0 and not pylibmc.support_compression:
    MIN_COMPRESS_LEN = 0
    warnings.warn(
        "A minimum compression length was provided but pylibmc was "
...
if not COMPRESS_LEVEL == -1:
    if not pylibmc.support_compression:
        warnings.warn(
            "A compression level was provided but pylibmc was "
            "not compiled with support for it."
        )
...
class PyLibMCCache(BaseMemcachedCache):
    ...
    def _cache(self):
        ...
        if self._options:
            log.info(f"Loading pylibmc options: {self._options}")
